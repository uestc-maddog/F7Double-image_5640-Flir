...
{ 	  
    ...
    {    
        ...
        GPIO_Initure.Pin=GPIO_PIN_2;           //PB5,背光控制
...
void LCD_two_Color_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 *color,u16 *color2)
{  
	u16 height,width;
	u16 i,j;
	u16 g,b;
	...
	for(i=0;i<height;i++)
	{
		for(j=0;j<width;j++)
		{
			r=(((color[i*width+j])&0xF800)+((color2[i*width+j])<<11));//写入数据 
			if(r > 0xf800 )
				r = 0xf800;
			else
				r = r & 0xf800;
			g=(u16)(((color[i*width+j])&0x07E0)+(((color2[i*width+j])<<6) + 0x20));//写入数据 
			if(g > 0x07E0 )
				g = 0x07E0;
			else
				g = g & 0x07E0;
			b=(u16)(((color[i*width+j])&0x001F)+(color2[i*width+j]));//写入数据 
			if(b > 0x001f)
				b = 0x001f;
			else
				b = b & 0x001f; 

			color2[i*width+j]=r+g+b;
			
		//	color2[i*width+j]=((color2[i*width+j])<<11) + (((color2[i*width+j])<<6) + 0x20) + color2[i*width+j];
		}
	}
...
//color:要填充的颜色
void LCD_Fill_DMA2D(u16 sx,u16 sy,u16 ex,u16 ey,u16 *color)
{
