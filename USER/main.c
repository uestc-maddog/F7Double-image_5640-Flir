...
void lepton_spi_rx_dma_cplt2(void);
extern u8 data_mask;
...
{ 
	...
	LCD_Clear(WHITE);
    POINT_COLOR=RED; 
	LCD_ShowString(30,50,200,16,16,"Doule image test!");
	//自动对焦初始化
		...
		if((xx[0][0] & 0x00ff) == 0)
		{
			...
			if((xx[59][0] & 0xff) == 59)
			{
				...
				for(i=0;i<60;i++)                   // Flir_Data         UDP Wifi_Data缓存
				{
					for(j=0;j<80;j++)
					{
						//sendbuf[sendfps][i*80+j+320*80] = xx[i][j+2];
...
//				}
				...
				LCD_ShowxNum(0,0,zhenshu,4,16,1);
...
{ 
	...
	LCD_ShowString(30,50,200,16,16,"Double Image");	
	LCD_Clear(RED); 	
	delay_ms(500);
